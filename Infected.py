...
class Infected(pygame.sprite.Sprite):
    images = [load_image('infected.png', -1),  # картинки спрайта
              load_image('man.png', -1)]

    def __init__(self, player, *group):  # инициализация спрайта
        ...
        self.x = choice([0, 900])
        self.y = choice([0, 700])
        if self.x == 0 and self.y == 0:  # определение направления
            self.route = 40
    ...
    def update(self, bullets_sprites, *args) -> None:  # обновление состояния
        self.x += math.sin(math.radians((self.route + 90) % 360)) * self.speed  # движение вперед
        self.y -= math.cos(math.radians((self.route + 90) % 360)) * self.speed
        if pygame.sprite.collide_mask(self, self.player):  # проверка столкновения с игроком
            self.x -= math.sin(math.radians((self.route + 90) % 360)) * self.speed  # остановка движения
            self.y += math.cos(math.radians((self.route + 90) % 360)) * self.speed
        self.rect.x = self.x
        self.rect.y = self.y
        ...
        if self.state == 0:
            ...
            if hit:  # обработка попадания пули
